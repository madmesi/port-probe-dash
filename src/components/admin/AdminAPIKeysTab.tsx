...
export const AdminAPIKeysTab = () => {
  ...
  const generateAPIKey = async () => {
    ...
    try {
      ...
      const key = `cmdb_${Math.random().toString(36).substring(2, 15)}${Math.random().toString(36).substring(2, 15)}${Math.random().toString(36).substring(2, 15)}`;
      const keyPrefix = key.substring(0, 12);
      
      // Hash the key (in production, this should be done server-side)
